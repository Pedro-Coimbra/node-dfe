...
class EnviaProcessor {
    ...
    async executar(documento, assincrono = false) {
        ...
        try {
            ...
            if (documento.docFiscal.modelo == '65') {
                let appendQRCode = this.appendQRCodeXML(documento, xmlAssinado, soapAutorizacao);
    ...
    configuraUrlsSefaz() {
        ...
        const soapAutorizacao = Sefaz.getSoapInfo(empresa.endereco.uf, ambiente, nfe_1.ServicosSefaz.autorizacao);
        const soapRetAutorizacao = Sefaz.getSoapInfo(empresa.endereco.uf, ambiente, nfe_1.ServicosSefaz.retAutorizacao);
        return {
            soapAutorizacao,
            soapRetAutorizacao
        };
    }
    appendQRCodeXML(documento, xmlAssinado, soapAutorizacao) {
        let qrCode = null;
    ...
    async enviarNF(xml, soapAutorizacao) {
    ...
    getDetProd(produto, ambiente, isPrimeiroProduto) {
        return {
            ...
            comb: produto.comb ? this.getComb(produto.comb) : null,
    ...
    getComb(combustivel) {
        ...
        if (combustivel) {
            let comb = {
                ...
                ...(combustivel.origComb ? { origComb: this.getOrigComb(combustivel.origComb) } : {}),
            };
            return comb;
        } else return {}
    ...
    getDetImposto(imposto, modelo, cfop) {
        let detImposto = {
            ...
            IBSCBS: imposto.ibscbs ? [this.getImpostoIBSCBS(imposto.ibscbs, modelo)] : [],
        ...
        switch (ibscbs.CST) {
            case '620':
                result = {
                    ...
                    ...(ibscbs.gIBSCBSMono && ibscbs.gIBSCBSMono.gMonoPadrao ? {
                        gIBSCBSMono: {
                            gMonoPadrao: {
                                ...(ibscbs.gIBSCBSMono.gMonoPadrao.qBCMono ? { qBCMono: ibscbs.gIBSCBSMono.gMonoPadrao.qBCMono } : {}),
                                ...(ibscbs.gIBSCBSMono.gMonoPadrao.adRemIBS ? { adRemIBS: ibscbs.gIBSCBSMono.gMonoPadrao.adRemIBS } : {}),
                                ...(ibscbs.gIBSCBSMono.gMonoPadrao.adRemCBS ? { adRemCBS: ibscbs.gIBSCBSMono.gMonoPadrao.adRemCBS } : {}),
                                ...(ibscbs.gIBSCBSMono.gMonoPadrao.vIBSMono ? { vIBSMono: ibscbs.gIBSCBSMono.gMonoPadrao.vIBSMono } : {}),
                                ...(ibscbs.gIBSCBSMono.gMonoPadrao.vCBSMono ? { vCBSMono: ibscbs.gIBSCBSMono.gMonoPadrao.vCBSMono } : {}),
                            },
                            vTotIBSMonoItem: ibscbs.gIBSCBSMono.vTotIBSMonoItem,
                            vTotCBSMonoItem: ibscbs.gIBSCBSMono.vTotCBSMonoItem,
                        }
                    } : {})
                };
                break;
            default:
                result = {
                    ...
                    ...(ibscbs.gIBSCBS ? {
                        gIBSCBS: {
                            vBC: ibscbs.gIBSCBS.vBC,
                            ...(ibscbs.gIBSCBS.vBC ? { vBC: ibscbs.gIBSCBS.vBC } : {}),
                            gIBSUF: this.getImpostoIBSUF(ibscbs.gIBSCBS.gIBSUF),
                            gIBSMun: this.getImpostoIBMun(ibscbs.gIBSCBS.gIBSMun),
                            vIBS: ibscbs.gIBSCBS.vIBS,
                            gCBS: this.getImpostoCBS(ibscbs.gIBSCBS.gCBS),
                        }
                    } : {})
                };
                break;
    ...
    getTotal(total) {
        return {
            ICMSTot: this.getIcmsTot(total.icmsTot),
            IBSCBSTot: this.getIbsCbsTot(total.ibsCbsTot),
            vNFTot: this.getVNFTot(total.vNFTot),
        };
    }
    getIbsCbsTot(ibsCbsTot) {
        return ibsCbsTot;
    }
    getVNFTot(vNFTot) {
        return vNFTot;
    ...
    getTransp(transp) {
        return {
            ...
            ...(transp.volumes ? { vol: this.getVolTransp(transp.volumes) } : {})
            /**
    ...
    getDetalhamentoPagamentos(pagamentos) {
        ...
        for (const pag of pagamentos) {
            detPag = {
                indPag: Utils.getEnumByValue(schema.TNFeInfNFePagDetPagIndPag, pag.indicadorFormaPagamento),
                tPag: Utils.getEnumByValue(schema.TNFeInfNFePagDetPagTPag, pag.formaPagamento),
                ...(pag.descricaoFormaPagamento ? { xPag: pag.descricaoFormaPagamento } : {}),
                vPag: pag.valor,
                ...(pag.dadosCartao ? { card: this.getDetalhamentoCartao(pag.dadosCartao) } : {})
            };
